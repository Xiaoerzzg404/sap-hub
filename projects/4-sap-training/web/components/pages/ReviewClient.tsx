...
export function ReviewClient({
  ...
  const favoritePhrases = useMemo(() => phrases.filter((phrase) => progress?.favoritePhrases.includes(phrase.id)).slice(0, 12), [phrases, progress]);
          ...
          <div className="mt-3 space-y-2">
            {favoriteTerms.length ? (
              favoriteTerms.map((term) => (
                <p key={term.id} className="rounded-md bg-mist p-2 text-sm">
                  <span lang="ja">{term.japanese}</span> · {term.chinese}
                </p>
              ))
            ) : (
              <p className="text-sm text-slate-500">暂无收藏术语。</p>
            )}
          </div>
        </section>
      </div>
