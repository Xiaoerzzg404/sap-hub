    ...
    <div className="rounded-lg border border-line bg-white p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <h3 lang="ja" className="mt-1 text-lg font-bold text-ink">
            {term.japanese}
          </h3>
          ...
          {term.reading ? (
            <p className="mt-1 text-xs text-slate-500">读法：{term.reading}</p>
          ) : null}
        </div>
        {term.needsReview ? (
      ...
      <p className="mt-2 text-sm text-ink">
        示例句：<span lang="ja">{followSentence}</span>
      </p>
      <div className="mt-3">
        <AudioPlayer
          sentences={[
      ...
      <button
        ...
        onClick={() => void toggleProgressList("favoriteTerms", term.id)}
      >
